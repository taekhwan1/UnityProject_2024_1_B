...
[Serializable]
public class Achievement                //업적 클래스 선언 (모노 비해이비어 X)
{
    ...
    public Achievement(string name, string description, int goal)    //생성 자 함수
    {
    ...
    public void AddProgress(int amount)              //업적 진행도 함수
    {
        if(!isUnlocked)                              //잠겨있지 않다면
        {
            ...
            {
                isUnlocked = true;
                OnAchievementUnlocked();                 //업적 달성 시 Debug.Log로 출력
            }
        }
    }
